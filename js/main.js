...
const executeCommand = (inputValue) => {
  ...
  switch (inputValue) {
    ...
    case commands[6]:
      ...
      break;
    case commands[7]:
      document.getElementById("reddit").click();
      cli.innerHTML += status;
      break;
...
const movefocus = (keycode) => {
  try {
    if (document.getElementById("0") !== null) {
      ...
      if (keycode === 38)
        focusedId = focusedId <= 0 ? highestId : focusedId - 1;
...
const recommendCommands = (inputValue) => {
  ...
  if (document.getElementById("0") !== null)
    document.getElementById("0").style.color = focusColor;
};
...
const initializeWinboxes = () => {
  about.addEventListener("click", () => {
    addWinBox("about", "50%", 50, 0, aboutContent);
  });
  contact.addEventListener("click", () => {
    addWinBox("contact", "50%", 80, 30, contactContent);
  });
  instructions.addEventListener("click", () => {
    addWinBox("instructions", "60%", 100, 60, instructionContent);
  });
};
...
const initializeTextReadOnly = () => {
  cliInput.addEventListener("keydown", (event) => {
    ...
    if (
      (which == 8 && cliInput.selectionStart <= readOnlyLength) ||
  ...
  cliInput.addEventListener("keypress", (event) => {
    var which = event.which;
    if (which != 0 && cliInput.selectionStart < readOnlyLength)
      event.preventDefault();
  });
...
const initializeDropdownMenu = () => {
  navBtn.addEventListener("click", function () {
    ...
    navLinks.forEach((navLink) => {
      navLink.className =
        navLink.className === "hidden-links" ? "" : "hidden-links";
    });
